...
RSpec.describe Clap::Exporter::Models::OntologyExporter do
  subject(:ontology) { described_class.new(ontology_id).to_clap }
  ...
  it 'exports a known ontology' do
    expect(ontology).to eq(id: 'ONT:1', label: 'Label', name: 'Name')
  end
  ...
  context 'with a missing ontology value' do
    ...
    it 'marks missing ontology values' do
      expect(ontology).to include(label: 'Error: Ontology specified but non-existant')
    end
  end
end
